function [expansionCoefficient,learningCurve] = ...
    SWKRLS2(K,trainInput,trainTarget,typeKernel,paramKernel,paramRegularization)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for n = K+1:trainSize
    ...
    s = 1/(paramRegularization+ ker_eval(trainInput(:,n),trainInput(:,n),typeKernel,paramKernel) - k_vector'*f_vector);
    Q_tmp = zeros(K+1,K+1);
    Q_tmp(1:K,1:K) = Q_matrix + f_vector*f_vector'*s;
    Q_tmp(1:K,K+1) = -f_vector*s;
    Q_tmp(K+1,1:K) = Q_tmp(1:K,K+1)';
    Q_tmp(K+1,K+1) = s;
    
    Q_matrix = Q_tmp(2:K+1,2:K+1) - Q_tmp(2:K+1,1)*Q_tmp(2:K+1,1)'/Q_tmp(1,1);
    
    expansionCoefficient = Q_matrix*trainTarget(n-K+1:n);
