function [expansionCoefficient,dictionaryIndex,learningCurve,CI] = ...
    AOGR_CI(trainInput,trainTarget,testInput,testTarget,typeKernel,paramKernel,regularizationFactor,forgettingFactor,th1,th2,inputApriori,flagLearningCurve)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Function AOGR
...
%
%inputApriori: -ln(input distribution)
%
...
CI(1) = log(predictionVar)/2 + predictionError^2/(2*predictionVar) + inputApriori(1);
...
for n = 2:trainSize
    ...
    CI(n) = log(predictionVar)/2 + predictionError^2/(2*predictionVar) + inputApriori(n);
    
    if (CI(n) > th1 && CI(n) < th2)
